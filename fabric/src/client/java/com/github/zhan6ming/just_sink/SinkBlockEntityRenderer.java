...
/**
 * 水槽液面渲染器 —— MC 1.21.11 版本。
 * <p>
 * 通过纹理图集直接获取原版水纹理（1.21.11 中无 FluidModel/FluidStateModelSet）。
...
public class SinkBlockEntityRenderer implements BlockEntityRenderer<SinkBlockEntity, SinkBlockEntityRenderer.SinkRenderState> {
    ...
    public void submit(
        ...
        // 通过 AtlasManager + Material 获取 MC 原版水纹理（1.21.11 API）
        Material waterMaterial = new Material(TextureAtlas.LOCATION_BLOCKS, Identifier.fromNamespaceAndPath("minecraft", "block/water_still"));
        TextureAtlasSprite waterSprite = Minecraft.getInstance().getAtlasManager().get(waterMaterial);
